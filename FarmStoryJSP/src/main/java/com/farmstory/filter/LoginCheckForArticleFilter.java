...
public class LoginCheckForArticleFilter implements Filter{
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1, FilterChain arg2) throws IOException, ServletException {
		...
		/*
		 * if(sessUser == null) { // 로그인을 하지 않으면 로그인 페이지로 이동
		 * 
		 * resp.sendRedirect("/jboard/user/login.do?success=102"); }else { // 로그인 상태이면
		 * 다음 필터 이동(Controller 요청) arg2.doFilter(arg0, arg1); }
		 */
	}
}
